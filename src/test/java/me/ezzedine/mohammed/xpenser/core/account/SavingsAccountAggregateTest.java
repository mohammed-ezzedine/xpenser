...
class SavingsAccountAggregateTest {
    private final AggregateTestFixture<SavingsAccountAggregate> testFixture = new AggregateTestFixture<>(SavingsAccountAggregate.class);
    ...
    void it_should_throw_an_error_upon_receiving_an_open_savings_account_command_with_a_negative_initial_budget() {
        testFixture.givenNoPriorActivity()
                .when(AccountUtils.openSavingsAccountCommand().budgetInitialAmount(BigDecimal.valueOf(-3)).build())
    ...
    void it_should_publish_an_account_opened_event_upon_receiving_an_open_savings_account_command() {
        testFixture.givenNoPriorActivity()
                .when(AccountUtils.openSavingsAccountCommand().build())
